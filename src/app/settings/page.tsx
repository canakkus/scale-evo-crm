...
import { Settings as SettingsIcon, Key, ShieldCheck, Database, Sparkles, CheckCircle2, XCircle } from "lucide-react";
    ...
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      <div>
        ...
        <p className="mt-1 text-xs" style={{ color: "var(--text-2)" }}>
          Übersicht der konfigurierten API-Keys, Auth-Berechtigungen und Datenbank-Einstellungen.
        </p>
      </div>
          ...
          <div className="divide-y" style={{ borderColor: "var(--border)" }}>
            ...
            <div className="py-3 flex items-center justify-between">
              <div>
                ...
                <p className="text-[11px]" style={{ color: "var(--text-3)" }}>Pflicht für Call-Transkription, KI-Assistent & Task-Priorisierung</p>
              </div>
              {data?.hasGeminiKey ? (
        ...
        <div className="rounded-xl border p-6 space-y-3" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            {data?.allowedEmails?.map((email: string, i: number) => (
              <span key={i} className="px-3 py-1 rounded-md text-xs font-mono border" style={{ background: "var(--surface-2)", borderColor: "var(--border)", color: "var(--accent)" }}>
                {email}
              </span>
            ))}
          </div>
